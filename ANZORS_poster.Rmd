...
################################################################################    
####     Un comment the below 2 lines for pdf rendering and make sure to 
####     comment out the self:contained: false above

    # self_contained: TRUE # need self contained for pdf rendering
# knit: pagedown::chrome_print # prints pdf version
...
# Introduction
Intramedullary (IM) nailing is the typical fixation for adult tibial shaft fractures, the most common long bone fracture [@Courtney2011]. Techniques to perform IM nailing vary, and studies comparing approaches show conflicting results. The absence of a validated, cohort-specific outcome measure has led to a wide range of outcome measures reported, making comparisons difficult. The true extent and prevalence of outcome measures in use is currently not known.
...
* 47% of papers were published within the last decade (Figure \@ref(fig:yearFig)):
  + 27 different countries of origin, most (35%) were from the USA. <font size="6"><em> (supplementary material)</em></font>

```{r prismaFig, fig.cap='PRISMA flowchart.', out.width = '100%'}
knitr::include_graphics(paste0(path.fig, "/prisma flow.png"))
```

```{r nailCountsShort, out.width='100%'}
...
nailCounts.print %>%
  kbl("html",
      booktabs = T,
      align = "lrrrrrr",
      caption = "Number of different IM nailing approaches. See supplementary material for more detailed breakdown.",
...
```{r yearFig, fig.cap='Number of publications per year grouped by IM nailing focus.'}
...
knitr::include_graphics(paste0(path.fig, "/CharacteristicDescriptions.png"))
```
...
* 47% of studies level were III evidence. <font size="6"><em> (supplementary material)</em></font>
...
This is the first study to review and chart the outcome measures used in the assessment of tibial shaft fractures treated with IM nailing. Notably, a number of the scores routinely used are designed for other pathologies and do not contain a kneeling component, shown to cause the most severe pain within this cohort [@Song2012], and the most frequent modality is simply asking whether any knee pain exists. Currently, no conclusive evidence exists to inform surgical decision making on whether an optimal IM nailing technique exists. This work highlights the need for a validated outcome measure designed specifically for this cohort.

### Acronyms
<!-- Need to add some css styling here to control the whitespace and font size  -->
<div style="margin-bottom:-13mm;">
</div>
<font size="6">
IM -- intramedullary; IPN -- infrapatellar nail; SE -- Semi-extended; SPN -- suprapatellar nail; ND -- no description; NA -- not applicable; ROM -- range of motion; VAS -- visual analogue score; NRS -- numerical rating score.</font>
